...
//image upload 2
        ...
        if($check !== false) {
                ...
                // echo "File is not an image.";
                $message .= base64_encode(urlencode("File 2 is not an image."));
                header('Location:create_event.php?msg=' . $message);
                $uploadOk = 0;
        }
        }
        ...
        if (file_exists($target_file2)) {
                $message .= base64_encode(urlencode("Sorry, file 2 already exists. "));
                header('Location:create_event.php?msg=' . $message);
        // echo "Sorry, file already exists.";
                $message .= base64_encode(urlencode("File is a Image"));
                header('Location:create_event.php?msg=' . $message);
        ...
        // echo "Sorry, your file is too large.";
                $message .= base64_encode(urlencode("Sorry, your file 2 is too large. "));
                header('Location:create_event.php?msg=' . $message);
        ...
        if ($uploadOk2 == 0) {
        echo "Sorry, your file was not uploaded.";
                $message .= base64_encode(urlencode("Sorry, your file was not uploaded. 2"));
                header('Location:create_event.php?msg=' . $message);
        // if everything is ok, try to upload file
        } else {
        if (move_uploaded_file($_FILES["fileToUpload2"]["tmp_name"], $target_file2)) {
                ...
                // echo "Sorry, there was an error uploading your file.";
                $message .= base64_encode(urlencode("Sorry, there was an error uploading your file. 2"));
                header('Location:create_event.php?msg=' . $message);
        }
        }		
